...
class Expense:
    ...
    def __init__(self):
        ...
        exp_window = Tk()
        exp_window.title('New expense')

        expense_discrimination_label = ttk.Label(exp_window, text='Discrimination: ')
        expense_discrimination_label.grid(column=0, row=0)
        expense_discrimination_entry = ttk.Entry(exp_window)
        expense_discrimination_entry.grid(column=1, row=0)

        expense_value_label = ttk.Label(exp_window, text='Value: ')
        expense_value_label.grid(column=0, row=1)
        expense_value_entry = ttk.Entry(exp_window)
        expense_value_entry.grid(column=1, row=1)

        expense_recurrent_label = ttk.Label(exp_window, text='Is it recurrent? ')
        ...
        recurrent = ttk.Combobox(exp_window, values=expense_recurrent_options, textvariable=expense_recurrent_var)
        recurrent.grid(column=1, row=2)

        expense_date_day_label = ttk.Label(exp_window, text='Day')
        ...
        day = ttk.Combobox(exp_window, values=expense_date_day_options, textvariable=expense_date_day_var)
        day.grid(column=1, row=3)

        expense_date_month_label = ttk.Label(exp_window, text='Month')
        ...
        month = ttk.Combobox(exp_window, values=expense_date_month_options, textvariable=expense_date_month_var)
        month.grid(column=1, row=4)

        expense_date_year_label = ttk.Label(exp_window, text='Year')
        ...
        year = ttk.Combobox(exp_window, values=expense_date_year_options, textvariable=expense_date_year_var)
        year.grid(column=1, row=5)

        expense_category_var_label = ttk.Label(exp_window, text='Category')
        ...
        category = ttk.Combobox(exp_window, values=expense_category_var_options, textvariable=expense_category_var)
        category.grid(column=1, row=6)

        expense_account_entry_label = ttk.Label(exp_window, text='Account')
        ...
        account = ttk.Combobox(exp_window, values=expense_account_var_options, textvariable=expense_account_var)
        account.grid(column=1, row=7)

        # submit button
        submit_button = ttk.Button(exp_window, text='Submit', command= Expense.submit)
        submit_button.grid(column=0, row=8, padx=10, pady=5)

        quit_button = ttk.Button(exp_window, text='Quit', command=exp_window.destroy)
